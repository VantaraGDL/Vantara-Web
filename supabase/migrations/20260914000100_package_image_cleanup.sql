...
create function public.reconcile_package_image(p_path text, p_finish boolean default false)
...
begin
 ...
 if exists(select 1 from public.packages where image_path=p_path) then
  if asset.state<>'ready' then raise exception 'La imagen está asignada a un paquete; no se eliminará' using errcode='22023'; end if;
...
-- Also protect the physical delete, including calls from older admin tabs.
