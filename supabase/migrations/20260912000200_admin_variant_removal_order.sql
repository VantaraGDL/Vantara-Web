begin;
-- Variant removals use {id, delete: true}; the RPC signature stays compatible.
...
begin
  ...
  if jsonb_array_length(p_variants)=0 or jsonb_array_length(p_variants)>6 then raise exception 'Selecciona variantes' using errcode='22023'; end if;
  ...
  for v in select value from jsonb_array_elements(p_variants) loop
    ...
    if jsonb_typeof(v->'size')<>'string' or s=any(sizes) or
      not(s=any(array['XS','S','M','L','XL','Única'])) then raise exception 'Talla inválida o duplicada' using errcode='22023'; end if;
    ...
    insert into public.product_variants(product_id,size,stock,position) values(new_id,s,n::integer,array_position(array['XS','S','M','L','XL','Única'],s)-1);
    i:=i+1;
  end loop;
  return new_id;
end;
...
begin
  ...
  update public.product_variants set position=coalesce(array_position(array['XS','S','M','L','XL','Única'],size)-1,position)
