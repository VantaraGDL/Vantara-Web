import {getPackageById} from './packages-api.js?v=packages-polish-1';
...
async function refresh(){
 ...
 packages=new Map(await Promise.all(ids.map(async id=>{
  try{return [id,await getPackageById(id,{fullGallery:false,products})];}
...
function externalChange(){if(mutating||busy)return;load();}
